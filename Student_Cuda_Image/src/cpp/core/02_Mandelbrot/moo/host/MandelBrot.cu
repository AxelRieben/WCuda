...
extern __global__ void mandelBrot(uchar4* ptrDevPixels, uint w, uint h, uint n, DomaineMath domaineMath, float t);
...
MandelBrot::MandelBrot(const Grid& grid, uint w, uint h, float dt, uint n, const DomaineMath& domaineMath) :
	Animable_I<uchar4>(grid, w, h, "Mandelbrot_Cuda_RGBA_uchar4", domaineMath), variateurAnimation(Interval<float>(30, n), dt)
    {
    // Inputs
    this->n = n;
...
void MandelBrot::process(uchar4* ptrDevPixels, uint w, uint h, const DomaineMath& domaineMath)
    ...
    mandelBrot<<<dg, db>>>(ptrDevPixels,w,h,n,domaineMath,t);
